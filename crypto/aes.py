"""
AES (Advanced Encryption Standard) Implementation from Scratch
Implements AES-128 encryption and decryption
"""

class AES:
    SBOX = [
        
    ]
    INV_SBOX = [
        
    ]
    RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]

    def __init__(self, key):
        if len(key) != 16:
            raise ValueError("Key must be 16 bytes (128 bits)")
        self.key = key
        self.round_keys = self._key_expansion(key)

    def _key_expansion(self, key):
        key_symbols = [b for b in key]
        expanded_key = []
        for i in range(4):
            expanded_key.append([key_symbols[4*i], key_symbols[4*i+1],
                               key_symbols[4*i+2], key_symbols[4*i+3]])
        for i in range(4, 44):
            temp = expanded_key[i-1][:]
            if i % 4 == 0:
                temp = temp[1:] + temp[:1]
                temp = [self.SBOX[b] for b in temp]
                temp[0] ^= self.RCON[i//4 - 1]
            expanded_key.append([expanded_key[i-4][j] ^ temp[j] for j in range(4)])
        round_keys = []
        for i in range(11):
            round_key = []
            for j in range(4):
                round_key.extend(expanded_key[i*4 + j])
            round_keys.append(round_key)
        return round_keys

    def _add_round_key(self, state, round_key):
        return [[state[i][j] ^ round_key[i*4 + j] for j in range(4)] for i in range(4)]

    def _sub_bytes(self, state):
        return [[self.SBOX[state[i][j]] for j in range(4)] for i in range(4)]

    def _inv_sub_bytes(self, state):
        return [[self.INV_SBOX[state[i][j]] for j in range(4)] for i in range(4)]

    def _shift_rows(self, state):
        return [
            [state[0][0], state[0][1], state[0][2], state[0][3]],
            [state[1][1], state[1][2], state[1][3], state[1][0]],
            [state[2][2], state[2][3], state[2][0], state[2][1]],
            [state[3][3], state[3][0], state[3][1], state[3][2]]
        ]

    def _inv_shift_rows(self, state):
        return [
            [state[0][0], state[0][1], state[0][2], state[0][3]],
            [state[1][3], state[1][0], state[1][1], state[1][2]],
            [state[2][2], state[2][3], state[2][0], state[2][1]],
            [state[3][1], state[3][2], state[3][3], state[3][0]]
        ]

    def _gmul(self, a, b):
        p = 0
        for _ in range(8):
            if b & 1:
                p ^= a
            hi_bit_set = a & 0x80
            a <<= 1
            if hi_bit_set:
                a ^= 0x1b
            b >>= 1
        return p & 0xFF

    def _mix_columns(self, state):
        result = [[0]*4 for _ in range(4)]
        for i in range(4):
            result[0][i] = self._gmul(0x02, state[0][i]) ^ self._gmul(0x03, state[1][i]) ^ state[2][i] ^ state[3][i]
            result[1][i] = state[0][i] ^ self._gmul(0x02, state[1][i]) ^ self._gmul(0x03, state[2][i]) ^ state[3][i]
            result[2][i] = state[0][i] ^ state[1][i] ^ self._gmul(0x02, state[2][i]) ^ self._gmul(0x03, state[3][i])
            result[3][i] = self._gmul(0x03, state[0][i]) ^ state[1][i] ^ state[2][i] ^ self._gmul(0x02, state[3][i])
        return result

    def _inv_mix_columns(self, state):
        result = [[0]*4 for _ in range(4)]
        for i in range(4):
            result[0][i] = self._gmul(0x0e, state[0][i]) ^ self._gmul(0x0b, state[1][i]) ^ self._gmul(0x0d, state[2][i]) ^ self._gmul(0x09, state[3][i])
            result[1][i] = self._gmul(0x09, state[0][i]) ^ self._gmul(0x0e, state[1][i]) ^ self._gmul(0x0b, state[2][i]) ^ self._gmul(0x0d, state[3][i])
            result[2][i] = self._gmul(0x0d, state[0][i]) ^ self._gmul(0x09, state[1][i]) ^ self._gmul(0x0e, state[2][i]) ^ self._gmul(0x0b, state[3][i])
            result[3][i] = self._gmul(0x0b, state[0][i]) ^ self._gmul(0x0d, state[1][i]) ^ self._gmul(0x09, state[2][i]) ^ self._gmul(0x0e, state[3][i])
        return result

    def _bytes_to_state(self, block):
        return [[block[i + 4*j] for j in range(4)] for i in range(4)]

    def _state_to_bytes(self, state):
        return bytes([state[i][j] for j in range(4) for i in range(4)])

    def encrypt_block(self, plaintext):
        if len(plaintext) != 16:
            raise ValueError("Block must be 16 bytes")
        state = self._bytes_to_state(plaintext)
        state = self._add_round_key(state, self.round_keys[0])
        for round_num in range(1, 10):
            state = self._sub_bytes(state)
            state = self._shift_rows(state)
            state = self._mix_columns(state)
            state = self._add_round_key(state, self.round_keys[round_num])
        state = self._sub_bytes(state)
        state = self._shift_rows(state)
        state = self._add_round_key(state, self.round_keys[10])
        return self._state_to_bytes(state)

    def decrypt_block(self, ciphertext):
        if len(ciphertext) != 16:
            raise ValueError("Block must be 16 bytes")
        state = self._bytes_to_state(ciphertext)
        state = self._add_round_key(state, self.round_keys[10])
        for round_num in range(9, 0, -1):
            state = self._inv_shift_rows(state)
            state = self._inv_sub_bytes(state)
            state = self._add_round_key(state, self.round_keys[round_num])
            state = self._inv_mix_columns(state)
        state = self._inv_shift_rows(state)
        state = self._inv_sub_bytes(state)
        state = self._add_round_key(state, self.round_keys[0])
        return self._state_to_bytes(state)

    def encrypt(self, plaintext):
        pad_len = 16 - (len(plaintext) % 16)
        plaintext = plaintext + bytes([pad_len] * pad_len)
        ciphertext = b''
        for i in range(0, len(plaintext), 16):
            ciphertext += self.encrypt_block(plaintext[i:i+16])
        return ciphertext

    def decrypt(self, ciphertext):
        if len(ciphertext) % 16 != 0:
            raise ValueError("Ciphertext length must be multiple of 16")
        plaintext = b''
        for i in range(0, len(ciphertext), 16):
            plaintext += self.decrypt_block(ciphertext[i:i+16])
        pad_len = plaintext[-1]
        return plaintext[:-pad_len]


def encrypt(plaintext, key):
    if isinstance(plaintext, str):
        plaintext = plaintext.encode('utf-8')
    if isinstance(key, str):
        key = key.encode('utf-8')
    if len(key) < 16:
        key = key.ljust(16, b'\x00')
    elif len(key) > 16:
        key = key[:16]
    cipher = AES(key)
    return cipher.encrypt(plaintext)


def decrypt(ciphertext, key):
    if isinstance(key, str):
        key = key.encode('utf-8')
    if len(key) < 16:
        key = key.ljust(16, b'\x00')
    elif len(key) > 16:
        key = key[:16]
    cipher = AES(key)
    return cipher.decrypt(ciphertext)
